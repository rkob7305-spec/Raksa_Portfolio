...
function ProjectHeader({ isDark }: { isDark: boolean }) {
  const title = "cambo TOURS";
  const badge = "CASE STUDY";
  const description = `I designed and created a tourism landing page website to promote travel experiences in Cambodia. The goal of this project was to build a visually attractive and user-friendly website that helps travelers explore destinations and easily book a tour.`;
...
function ProjectShowcase({ isDark }: { isDark: boolean }) {
  return (
    <section className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 lg:px-14 py-8 md:py-12 space-y-8 md:space-y-12">
      <ModernAppCard isDark={isDark} />
    </section>
  );
}

// Modern App Card
function ModernAppCard({ isDark }: { isDark: boolean }) {
  return (
    <motion.div
      ...
      style={{ 
        ...
        boxShadow: "0px 4px 9.4px rgba(0, 0, 0, 0.25)",
        border: "1px solid rgba(255, 255, 255, 0.2)",
        aspectRatio: "1328/1344",
        minHeight: "600px"
      }}
    >
      <motion.div
      ...
      >
        <div className="relative w-full h-full">
          <Image
            src={isDark ? "/img/book1.png" : "/img/book2.png"}
            alt="Booking Website"
            fill
