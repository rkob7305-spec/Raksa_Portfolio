...
function ProjectHeader({ isDark }: { isDark: boolean }) {
  const title = "After High School";
  const badge = "Project competition";
  const description = `After High School is an educational app that guides students in exploring universities, selecting majors, and learning about the job market in Cambodia. It helps users understand career opportunities and the skills needed for their future.`;
  const subDescription = `I led the UX/UI design process, from user research and user flows to wireframes, prototyping, and testing. My focus was to create a clean, intuitive interface that delivers a seamless user experience.`;
  const meta = [{ label: "Role", value: "Project Lead | UX/UI Design | UX Research" }];
          ...
          >
            {meta.map((item) => (
              <React.Fragment key={item.label}>
                ...
                <span
                  className="text-[14.5px] italic tracking-[0.01em] whitespace-nowrap"
...
function ProjectShowcase({ isDark }: { isDark: boolean }) {
  return (
    <section className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 lg:px-14 py-8 md:py-12 space-y-8 md:space-y-12">
      ...
      <CombinedTextCard isDark={isDark} />
    </section>
  );
}
    ...
    >
      <div className="absolute inset-0 flex items-center justify-center p-8">
        <div className="relative w-full h-full">
          <Image
            src={isDark ? "/img/Frame 575.png" : "/img/eucation8.png"}
...
// Combined Text Card - UX/UI APP DESIGN PROCESS and PROJECT TIMELINE
function CombinedTextCard({ isDark }: { isDark: boolean }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 80 }}
      whileInView={{ opacity: 1, y: 0 }}
      ...
      className="w-full max-w-[1328px] mx-auto rounded-[5px] relative overflow-hidden flex flex-col items-center justify-start gap-4 md:gap-6 lg:gap-8 py-8 md:py-12 lg:py-16 px-4 md:px-8"
      style={{ 
        background: "rgba(255, 255, 255, 0.1)",
        border: "1px solid rgba(255, 255, 255, 0.2)",
        boxShadow: "0px 4px 9.4px rgba(0, 0, 0, 0.15)",
    ...
    >
      <h3 
        className={`font-bold text-center uppercase tracking-wider ${isDark ? 'text-white' : 'text-black'}`}
        style={{
      ...
      >
        UX/UI APP DESIGN PROCESS
      </h3>
      
      {/* Image below UX/UI text */}
      <div className="relative w-full max-w-[1000px]" style={{ aspectRatio: "16/9" }}>
        <Image
          src={isDark ? "/img/eucatin4.png" : "/img/eucation5.png"}
      ...
      <h3 
        className={`font-bold text-center uppercase tracking-wider ${isDark ? 'text-white' : 'text-black'} mt-4 md:mt-8 lg:mt-12`}
        style={{
          fontFamily: 'Unbounded, sans-serif',
          fontSize: 'clamp(16px, 2.5vw, 32px)',
          letterSpacing: '0.1em'
      ...
      {/* Image below PROJECT TIMELINE text */}
      <div className="relative w-full max-w-[1000px]" style={{ aspectRatio: "16/9" }}>
        <Image
          src={isDark ? "/img/eucation6.png" : "/img/eucation4.png"}
